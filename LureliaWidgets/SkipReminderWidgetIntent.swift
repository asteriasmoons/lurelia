//
//  SkipReminderWidgetIntent.swift
//  Lurelia
//
...
struct SkipReminderWidgetIntent: AppIntent {
    static var title: LocalizedStringResource = "Skip Reminder"
    static var description = IntentDescription("Skips a due reminder from the widget.")
    ...
    func perform() async throws -> some IntentResult {
        ...
        skipReminderOccurrence(reminder, in: context)
        try context.save()
    ...
    private func skipReminderOccurrence(_ reminder: LureliaReminder, in context: ModelContext) {
        let skippedAt = exactFireTime(from: reminder.nextFireAt ?? reminder.scheduledDate)

        // Record skipped timestamp
        var timestamps = reminder.skippedTimestamps
        if !timestamps.contains(where: { historyOccurrenceKey($0) == historyOccurrenceKey(skippedAt) }) {
        ...
        // For non-repeating reminders, mark as completed (skipped = dismissed)
        if reminder.repeatUnit == .none {
            reminder.isCompleted = true
            reminder.completedAt = skippedAt
            return
        }

        // For repeating reminders, advance to next occurrence
        if let next = nextScheduledDate(for: reminder, after: skippedAt) {
